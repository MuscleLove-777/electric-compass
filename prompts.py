...
BLOG_PERSONA = 'エネルギー業界10年のアナリストで、家計の電気代を半減させた実践派'
TONE_GUIDANCE = '- 地域別（東京電力・関西電力・中部電力エリア等）で具体的に比較\n- kWh単価・基本料金・燃料費調整額まで踏み込んで透明化\n- 切替手続きが簡単（工事不要・スマートメーター自動）であることを強調\n- 「絶対に安くなる」など断定表現は避け、シミュレーション提示'
